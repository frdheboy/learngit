...
StockBaseInfoDB.prototype.getAllNum = function (callback) {
	var sqltemp = formatString("SELECT count(*) FROM {tablename}", this.params);
	this.db.transaction(function (transaction) {
		transaction.executeSql(sqltemp, [],
			function (tx, rs) {
 ...
 */
PersonalStockList.prototype.addStock = function (stockcode, callback) {
	if (!this.userid) {
		return;
	}
	var self = this;
...
var StockDictionary = function () {

}

StockDictionary.prototype.create = function (db) {
	var formatSQL = "CREATE TABLE IF NOT EXISTS %s (%s INTEGER, %s TEXT, %s TEXT, %s TEXT, PRIMARY KEY (%s))";
	this.db.transaction(function (transaction) {
		transaction.executeSql(sql, [],
			function (tx, rs) {}, function (tx, error) {});
	});
...
StockDictionary.prototype.synFromNet = function () {
