		...
		W.parseCMD('/users/show.json', function (oResult, bStatus) {
			...
			if (bStatus) {
				requestThirdPartLogin({
					...
					type: me.platform,
					token: me.accesstoken,
					name: oResult.screen_name,
					head: oResult.profile_image_url
				});
			}
		}, {
			uid: me.openid
 ...
 *三方登录失败
 *  */
function login_failed(data, thirdpartinfo) {
	if (data && data.status == "1010") { //账户未绑定
		window.location.href = 'thirdpartysignon.html?nickname=' + thirdpartinfo.name + '&openid=' + thirdpartinfo.openid + '&head=' + thirdpartinfo.head + '&type=' + thirdpartinfo.type;
	} else {
		alert("授权失败");
	}
};
